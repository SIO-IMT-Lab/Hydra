...
constexpr int SAMPLE_PIN = 6; // HIGH = sampling motor active
constexpr int FLUSH_PIN = 5;  // HIGH = flushing motor active
constexpr int STATUS_PIN = 1; // Sample State = HIGH (Green LED), Flush State = LOW (Red LED)
...
void loop()
{
  ...
  if (range1 > SWSW_THRESHOLD)
    Serial.println("SWSW 1 Shorted");
  if (range2 > SWSW_THRESHOLD)
